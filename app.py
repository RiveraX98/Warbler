...
def update_profile(user_id):
    ...
    if g.user:
        ...
        if form.validate_on_submit():
            auth_user = User.authenticate(user.username,
                                          form.password.data)
            if auth_user:
...
def delete_user():
    ...
    msgs = Message.query.filter_by(user_id=g.user.id).delete()
...
def handle_likes(message_id):
    """add Like to a message or unlike an already liked message"""
    user = g.user
    like = Likes.query.filter_by(
        user_id=user.id, message_id=message_id).first()
...
def show_liked_messaged(user_id):
    """Show all messages liked by the user"""
    user = g.user
    messages = user.likes
...
def homepage():
    ...
    - logged in and following 0: 100 most recent messages in db
    """
    user = g.user
    if user and len(user.following) > 0:
        msgs = []
